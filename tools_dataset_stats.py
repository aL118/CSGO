...
save_path = 'D:\\lyzheng\\projects\\angela\\Counter-Strike_Behavioural_Cloning\\stats\\'
# figsize_in = (5,2.5) # x, y width

is_save=True

file_name_stub = 'dm_july2021_' # 'my_currvarsv2_bot1_to_100' 'dm_july2021_
...
# for file_chunk in range(0,55):
    ...
    if False:
        name = folder_name+file_name_stub+".npy"
    else:
        name = folder_name+'currvarsv2_'+file_name_stub+str(file_chunk*n_filer_per_chunk+1)+'_to_'+str((file_chunk+1)*n_filer_per_chunk)+'.npy'
    dict_chunk = np.load(name,allow_pickle=True)
    ...
    for file_i in range(file_chunk*n_filer_per_chunk+1,2):
        for frame_i in range(0,min(406,len(dict_chunk))):
            key = 'file_num' + str(file_i) +'_frame_' + str(frame_i)

            dict_key = dict_chunk[key]
            if False:
                mousex = dict_key[0]['mousex']
                mousey = dict_key[0]['mousey']
            else:
                mousex = dict_key[1][1]
                mousey = dict_key[1][2]
...
ax.scatter(info_array[:2000000,0],info_array[:2000000,1],alpha=0.2,s=4,lw=0.,color='magenta')
